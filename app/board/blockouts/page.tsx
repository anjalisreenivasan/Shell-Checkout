  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-shell-black">Blockouts</h2>
        <Button onClick={() => setShowForm(true)} size="sm" className="bg-shell-black hover:bg-shell-black/80 text-white gap-1.5">
          <Plus className="w-3.5 h-3.5" /> Block Time
        </Button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-shell-black/10 p-5 space-y-4">
          <span className="text-sm font-medium text-shell-black">New Blockout</span>

          <div className="space-y-1.5">
            <Label className="text-sm text-shell-black/60">Item <span className="text-shell-red">*</span></Label>
            <select
              value={formItemId}
          ...
          <div className="space-y-1.5">
            <Label className="text-sm text-shell-black/60">Reason <span className="text-shell-red">*</span></Label>
            <Input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-sm text-shell-black/60">Start Date <span className="text-shell-red">*</span></Label>
              <Input type="date" value={formStartDate} onChange={e => setFormStartDate(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm text-shell-black/60">Start Time <span className="text-shell-red">*</span></Label>
              <select
                value={formStartTime}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-sm text-shell-black/60">End Date <span className="text-shell-red">*</span></Label>
              <Input type="date" value={formEndDate} onChange={e => setFormEndDate(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm text-shell-black/60">End Time <span className="text-shell-red">*</span></Label>
              <select
                value={formEndTime}
          ...
          <div className="flex gap-2 pt-1">
            <Button type="submit" disabled={submitting} size="sm" className="bg-shell-black hover:bg-shell-black/80 text-white">
            ...
            <div key={b.id} className="flex items-center justify-between gap-4 px-4 py-3.5">
              <div className="min-w-0 space-y-0.5">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className="bg-shell-black text-white border-shell-black text-[10px]">
                    Blocked
                  </Badge>
                  <span className="font-medium text-sm text-shell-black truncate">
                    {(b.item as { name: string })?.name}
                  </span>
                </div>
                <p className="text-xs text-shell-black/50">{b.title}</p>
                <p className="text-xs text-shell-black/40 flex items-center gap-1">
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-shell-black/50">
            Remove the blockout for <span className="font-medium text-shell-black">{(deleteTarget?.item as { name: string })?.name}</span>?
          </p>
          <DialogFooter>
